...
public class HomeController {
    ...
    private void createMovies() {
        TransactionStatus moviesTransaction = moviesTransactionManager.getTransaction(null);
        for (Movie movie : movieFixtures.load()) {
            moviesBean.addMovie(movie);
        }
        moviesTransactionManager.commit(moviesTransaction);
    }

    private void createAlbums() {
        TransactionStatus albumsTransaction = albumsTransactionManager.getTransaction(null);
        for (Album album : albumFixtures.load()) {
            albumsBean.addAlbum(album);
        }
        albumsTransactionManager.commit(albumsTransaction);
    }
}
